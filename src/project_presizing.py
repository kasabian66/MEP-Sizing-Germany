...
# --- German market-style specific load benchmarks (pre-sizing) ---
# These are rule-of-thumb values for early sizing. They are NOT DIN EN 12831 or VDI 2078 results.
MARKET_HEAT_WM2 = {
    "Office": {"Existing building": 89.0, "New build (GEG baseline)": 59.0, "High performance (indicative)": 45.0},
    "Retail": {"Existing building": 56.0, "New build (GEG baseline)": 54.0, "High performance (indicative)": 45.0},
    "Hotel": {"Existing building": 100.0, "New build (GEG baseline)": 77.0, "High performance (indicative)": 60.0},
    "Residential": {"Existing building": 70.0, "New build (GEG baseline)": 40.0, "High performance (indicative)": 30.0},
    "Parking": {"Existing building": 0.0, "New build (GEG baseline)": 0.0, "High performance (indicative)": 0.0},
}

# Cooling: rule-of-thumb W/m² values for quick AC sizing (pre-sizing).
MARKET_COOL_WM2 = {
    "Office": 110.0,
    "Retail": 120.0,
    "Hotel": 90.0,
...
def suggest_specific_loads_wm2(use_type: str, geg_context: str, design_temp_C: float, design_summer_C: float) -> Dict[str, float]:
    """Suggest heating/cooling specific loads (W/m²) using German market benchmarks + climate sensitivity (pre-sizing)."""
    use_type = str(use_type)
    geg_context = str(geg_context)

    heat_base = float(MARKET_HEAT_WM2.get(use_type, MARKET_HEAT_WM2["Office"]).get(geg_context, 59.0))
    cool_base = float(MARKET_COOL_WM2.get(use_type, MARKET_COOL_WM2["Office"]))

    heat_factor = float(climate_adjust_heating_factor(float(design_temp_C)))
    cool_factor = float(climate_adjust_cooling_factor(float(design_summer_C)))

    return {
        "heat_base_W_m2": heat_base,
        "cool_base_W_m2": cool_base,
        "heat_factor": heat_factor,
        "cool_factor": cool_factor,
        "heat_adj_W_m2": max(0.0, heat_base * heat_factor),
        "cool_adj_W_m2": max(0.0, cool_base * cool_factor),
    }
